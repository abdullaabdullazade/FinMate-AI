...
async def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    recent_expenses = db.query(Expense).filter(
        Expense.user_id == user.id,
        Expense.amount.isnot(None)  # Filter out expenses with None amounts
    ).order_by(Expense.created_at.desc()).limit(10).all()
    
    # Ensure all expense amounts are valid (safety check)
    for expense in recent_expenses:
        if expense.amount is None:
            expense.amount = 0.0
        # Also ensure items have valid prices
        if expense.items:
    ...
    if user.daily_budget_limit:
        ...
        today_total = sum(exp.amount or 0.0 for exp in today_expenses)
    ...
    try:
        ...
        if user.last_rewarded_month != last_month_str:
            ...
            last_month_spending = sum(exp.amount or 0.0 for exp in last_month_expenses)
    ...
    for expense in recent_expenses[:5]:  # Check last 5 expenses
        ...
        amount = expense.amount or 0.0  # Ensure amount is not None
