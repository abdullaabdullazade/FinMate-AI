 ...
 * Scan və Mikrofon funksiyaları ilə
 */
...
import '../../styles/components/chat/chat-actions.css'
...
const ChatInput = ({ inputMessage, setInputMessage, handleSendMessage, loading, inputRef, disabled = false }) => {
  ...
  // Scan funksiyası - Scan səhifəsinə yönləndir
  const handleScan = () => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <form
        ...
        className="glass-card p-3 sm:p-4 flex items-center gap-2 sm:gap-3 mx-auto"
        id="chat-form"
        style={{ maxWidth: '100%' }}
      >
        {/* Scan Button */}
        <button
          type="button"
          onClick={handleScan}
          disabled={loading || disabled || isScanning}
          className={`chat-action-btn ${isScanning ? 'active' : ''}`}
          title="Scan et"
        >
