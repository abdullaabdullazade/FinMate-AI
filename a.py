...
def get_current_user(request: Request, db: Session = Depends(get_db)) -> Optional[User]:
    """
    MODIFICATION: Login səhifəsini ləğv edir.
    Həmişə 'demo' istifadəçisi kimi avtomatik giriş edir.
    """
    # Həmişə 'demo' istifadəçisini axtarırıq
    user = db.query(User).filter(User.username == "demo").first()
    
    # Əgər demo istifadəçi yoxdursa, yaradırıq (ilk işə düşəndə xəta verməsin deyə)
    if not user:
        user = User(
            username="demo", 
            monthly_budget=2000.0, 
            password_hash=hash_password("demo"), # Formal olaraq qalır
            currency="AZN",
    ...
    # Sessiyaya da yazırıq ki, bəzi middleware-lər işləsin
