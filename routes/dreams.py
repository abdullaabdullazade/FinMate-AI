...
from models import User, Expense, Dream, Income
...
async def add_dream_savings(
    ...
    try:
        ...
        actual_amount_to_deduct = min(amount_azn, remaining_needed) if remaining_needed > 0 else 0
        
        if actual_amount_to_deduct <= 0:
            return JSONResponse({
                "success": False,
                "error": "Bu arzu artıq tamamlanıb!"
            }, status_code=400)
